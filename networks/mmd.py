...
class UNet_MMD(nn.Module):
    ...
    def train_epoch(self, loader_src, loader_tar, loss_fn, lambdas, optimizer, epoch, print_stats=1, writer=None,
        ...
        for i, data in enumerate(loader_src):
            ...
            total_loss = loss + total_loss_mmd
            ...
            if i % print_stats == 0:
                print('[%s] Epoch %5d - Iteration %5d/%5d - Loss: %.6f - Loss MMD: %.6f - Total loss: %.6f'
                      % (datetime.datetime.now(), epoch, i, len(loader_src.dataset), loss, total_loss_mmd, total_loss))
        ...
        print('[%s] Epoch %5d - Train averages: Loss: %.6f - Loss MMD: %.6f - Total loss: %.6f'
              % (datetime.datetime.now(), epoch, loss_avg, total_loss_mmd_avg, total_loss_avg))
        ...
        if writer is not None:
            ...
            for i in range(len(lambdas)):
                writer.add_scalar('train/loss_mmd_level_' + str(i), losses_mmd_avg[i], epoch)
            writer.add_scalar('train/total_loss_mmd', total_loss_mmd_avg, epoch)
